...
- [x] Site and tenant contracts: `Tenant`, `Site`, `Customer` value objects; every provider instance, asset, health record, alert and action request carries a `tenantRef` — _`ServiceInstance.tenantRef`, `ControlledActionRequest.tenantRef` and `ActionAuditRecord.tenantRef` are literal fields. `ProviderHealth`/`ProviderEvent`/`ProviderResource`/`ProviderDiagnostic` are not, by design — tenant identity is instead recoverable via a same-refresh `instanceId → tenantRef` map (the mechanism `CanonicalAsset.tenantRef` has used since #72). A first audit claiming this fully closed the line was corrected by review, twice: (1) a real bug, now fixed — `buildSnapshot`/`performRefresh` deduplicated alerts on `eventId` alone, which several providers construct without embedding `instanceId` (e.g. Uptime Kuma's `"monitor:$monitorId:down"`), so two different instances' identically-numbered monitors collided and one's alert was silently dropped; deduped on `(instanceId, eventId)` instead, matching how asset dedup already worked. (2) a real gap, now closed — the flat Health/Alerts/Assets/Diagnostics tabs and Search rendered an all-tenants-mode result with no tenant label at all, unlike the three correlation tabs. Every `OperationCard`/`OperationsCard` row on both platforms now takes an optional tenant-label badge next to its title, shown only in all-tenants mode and only when more than one tenant is actually present in the current refresh (hidden otherwise, same rule as every other all-tenants-only affordance) — Search inherits it for free since it renders through the same card functions. The Settings → Action History screen had the identical gap (fanning out across every tenant's audit ledger and durable queue in all-tenants mode with no indication of which tenant a row belonged to) and got the same fix: `RowHeader` on both platforms takes the same optional badge, resolved directly from `ActionAuditRecord.tenantRef`/`ControlledActionRequest.tenantRef` (both literal fields already, so no `instanceId` lookup needed here) under the identical gating rule_
