...
- [ ] Tenant switcher and scoping UI on both clients; global workspace defaults to the active tenant, with an explicit all-tenants mode for single-tenant installs — _in progress: a Settings → Tenants screen (add/rename/delete/activate) and a tenant picker on the instance create/edit flow (hidden on single-tenant installs, defaults to the active tenant) landed on both platforms, so `ServiceInstance.tenantRef` is finally user-assignable; still missing: a switcher in the global workspace chrome and an all-tenants fan-out mode_
