...
- [ ] Tenant switcher and scoping UI on both clients; global workspace defaults to the active tenant, with an explicit all-tenants mode for single-tenant installs — _in progress: a Settings → Tenants screen (add/rename/delete/activate, backed by `TenantStore`) landed on both platforms; still missing: a switcher in the global workspace chrome, all-tenants mode, and a tenant picker on the instance create/edit flow, so `ServiceInstance.tenantRef` still can't be set to anything but `default` through the UI_
