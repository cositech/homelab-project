...
Nginx Proxy Manager is the first configuration-provider migration. Enabling a proxy host is low risk; disabling is medium risk and requires a dedicated confirmation because it stops traffic; creation, editing and deletion are high risk because they can expose, reroute or remove public services. Android and iOS use normalized proxy-host identities, declare Nginx Proxy Manager write capability and execute all five mutations through the shared audit path. Form Save and delete confirmation provide the explicit confirmation for high-risk actions. Mutation requests never replay against the fallback URL, and indeterminate outcomes are non-retryable because Nginx Proxy Manager does not expose a mutation idempotency token.
