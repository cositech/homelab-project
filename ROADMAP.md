...
- [ ] Site and tenant contracts: `Tenant`, `Site`, `Customer` value objects; every provider instance, asset, health record, alert and action request carries a `tenantRef` — _`ServiceInstance.tenantRef`, `ControlledActionRequest.tenantRef` and `ActionAuditRecord.tenantRef` are literal fields. `ProviderHealth`/`ProviderEvent`/`ProviderResource`/`ProviderDiagnostic` are not, by design — tenant identity is instead recoverable via a same-refresh `instanceId → tenantRef` map (the mechanism `CanonicalAsset.tenantRef` has used since #72), and every correlation tab (By Asset/By Site/By Customer) already renders it. An audit claiming this fully closes the line was corrected by review: (1) a real bug, now fixed — `buildSnapshot`/`performRefresh` deduplicated alerts on `eventId` alone, which several providers construct without embedding `instanceId` (e.g. Uptime Kuma's `"monitor:$monitorId:down"`), so two different instances' identically-numbered monitors collided and one's alert was silently dropped — worse in all-tenants mode, but reproducible with two instances of the same provider in a single tenant too; deduped on `(instanceId, eventId)` instead, matching how asset dedup already worked. (2) a real, still-open gap — the flat Health/Alerts/Assets/Diagnostics tabs and Search render an all-tenants-mode result with no tenant label at all, unlike the three correlation tabs, so two tenants' identically-configured resources are visually indistinguishable there; this is the concrete remaining piece, not yet scheduled_
